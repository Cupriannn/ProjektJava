...
public class RezerwacjaManager {
    ...
    public void usunBilet(Scanner scanner, LotManager lotManager) {
    ...
    try {
        ...
        while (it.hasNext()) {
            ...
            if (r.id == id) {
                // Przywróć miejsca do lotu
                Lot lot = lotManager.znajdzLot(r.numerLotu);
                if (lot != null) {
                    lot.iloscMiejsc += r.iloscMiejsc;
                }
                it.remove();
        ...
        if (!znaleziono) {
            System.out.println("Nie znaleziono rezerwacji o podanym ID: " + id);
        }
